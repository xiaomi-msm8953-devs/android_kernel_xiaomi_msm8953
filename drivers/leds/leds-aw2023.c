...
static void aw2023_brightness_work(struct work_struct* work) {
    struct aw2023_led* led = container_of(work, struct aw2023_led, brightness_work);
    u8 val;
...
static ssize_t aw2023_led_time_show(struct device* dev, struct device_attribute* attr, char* buf) {
    struct led_classdev* led_cdev = dev_get_drvdata(dev);
    struct aw2023_led* led = container_of(led_cdev, struct aw2023_led, cdev);
...
static ssize_t aw2023_led_time_store(struct device* dev, struct device_attribute* attr,
                                     const char* buf, size_t len) {
    struct led_classdev* led_cdev = dev_get_drvdata(dev);
    struct aw2023_led* led = container_of(led_cdev, struct aw2023_led, cdev);
...
static struct attribute_group aw2023_led_attr_group = {.attrs = aw2023_led_attributes};
...
static int aw2023_led_err_handle(struct aw2023_led* led_array, int parsed_leds) {
    int i;
    /*
    ...
    for (i = 0; i < parsed_leds; i++) {
        sysfs_remove_group(&led_array[i].cdev.dev->kobj, &aw2023_led_attr_group);
...
static int aw2023_led_parse_child_node(struct aw2023_led* led_array, struct device_node* node) {
    ...
    for_each_child_of_node(node, temp) {
        ...
        rc = sysfs_create_group(&led->cdev.dev->kobj, &aw2023_led_attr_group);
        if (rc) {
...
static int aw2023_led_probe(struct i2c_client* client, const struct i2c_device_id* id) {
